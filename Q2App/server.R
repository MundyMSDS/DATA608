...
shinyServer(function(input, output, session) {
  ...
  output$cancerTypeUi <- renderUI({
    selectizeInput("cancerType", "",
                   ...
                   selected = "Neoplasms", multiple = TRUE,
  ...
  output$variablesUi <- renderUI({
    selectizeInput("variablesSelect", "Variables to show:",
                   ...
                   selected = "crude_rate", multiple = TRUE,
                   #selected = unique(cDatRaw$stat), multiple = TRUE,
                   options = list(placeholder = "Select variables to show"))
  })
  
  # Create select box input for choosing states
  output$stateTypeUi <- renderUI({
    selectizeInput("state", "",
                   levels(cDatRaw$state),
                   selected = list("US","NY"), multiple = TRUE,
  ...
  cDat <- reactive({
    ...
    isolate({
      ...
      # Filter state
      if (input$subsetType2 == "specific" & !is.null(input$state)) {
        data %<>%
  ...
  buildPlot <- reactive({
    ...
    p <-
      ...
      geom_line() +
      facet_wrap(~ state) +
      guides(color = guide_legend(title = "Cause",
                                  ncol = 3,
